...
1. **직시와 외면** — 대응이 정반대인 두 부류가 같은 골목에 있다.
   **흔적**(핏자국·백골·손자국)은 무섭지만 **다가가서 똑바로 봐야** 사라지고,
   **사람 형태**(창문의 얼굴·그네의 형체·어둠 속 눈)는 **절대 보면 안 된다** —
   마주친 채 버티면 붙잡힌다. "저건 흔적인가, 사람인가"를 판단하는 순간이 이 게임의 긴장
...
모바일: "걷는다" 버튼을 누르는 동안 전진, 화면 드래그로 시점, 탭으로 직시.
달리기 없음 — 속도는 하나뿐이다.
