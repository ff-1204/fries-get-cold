...
1. ⭐ **동사는 걷기 하나다** — 짚지 않고, 조사하지 않고, 싸우지 않는다.
   이상현상은 처리할 과제가 아니라 **지나가는 풍경**이다. 없앨 수 없으므로 계속 거기 있다
2. ⭐ **걸으면 줄고, 멈추면 자란다** — 무서운 것을 보면 사람은 멈추고,
   멈춰 있는 동안 앞의 골목이 실제로 길어진다. 가로등이 하나 어두워지고, 지나온 담장이
   한 구간 더 붙는다. **게임이 벌하는 것이 아니라 반응이 벌한다** — 규칙을 배울 필요가 없다
3. **끝나지 않는 밤** — 틀려도 죽지 않는다. 대신 같은 구간이 미묘히 달라진 채 반복되고,
   **어긋남은 하나 더 늘어난다(증식)**
4. **퇴근길 튜토리얼 + 귀갓길 구조** — 밝은 두 구간에서 길과 조작을 익히고,
