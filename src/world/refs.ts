...
export interface CorridorRefs {
   ...
   *  (좌우 되돌리기는 애초에 필요 없었고, 이제 반전 자체가 없다 — runtime.ts 폐기 주석) */
  banner: THREE.Mesh;
...
export interface Theme5Refs {
  ...
  // (`lettering` — 좌우 반전 때 글씨를 되돌리는 배열이었다. 반전 자체를 걷어내면서 함께 지웠다.
  //  ⭐ 이제 **벽에 글씨를 붙일 때 아무 예외 처리도 필요 없다** — 맵이 뒤집히지 않으므로
  //  텍스처도 뒤집히지 않는다. runtime.ts의 폐기 주석 참조)
