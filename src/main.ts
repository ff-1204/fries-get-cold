...
// ⭐ **판정이 하나도 남지 않았다** (v0.11.56 — M3 전환 완료: 응시 붙잡힘까지 제거).
// 남은 인과는 **머무름 → 자람** 하나뿐이다 (config.stillGrowSec). 차도의 치임은 판정이 아니라
// 골목의 사물이 하는 일이고, 대가는 늘어남으로 같다
...
// ⭐ **가르칠 것이 하나뿐이다** — 직시 힌트는 v0.11.50, 외면 힌트는 v0.11.56에 사라졌다.
// 동사가 걷기 하나뿐이라 온보딩도 걷기 힌트 하나로 끝난다.
...
// 여기 남은 것은 온보딩 힌트와 검증 훅이 쓰는 계산뿐이다 (응시 판정도 v0.11.56에 사라졌다)
...
function updateWalk(dt: number) {
  ...
  // 관찰을 방해하는 것이 하나도 돌지 않아야 구조를 볼 수 있다
  if (admin.active) {
