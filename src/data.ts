...
/** 부류 — 예전에는 '통과 규칙'이었다 (v0.11.0~49).
 *
 *  ⚠ **gaze는 더 이상 규칙이 아니다** (v0.11.50): 짚는 동사가 사라져 흔적은 그냥 풍경이다.
 *  남은 쓰임은 둘 — ① 증식이 부류가 다른 것부터 번갈아 뽑는 기준(시선 높이를 나눈다)
 *  ② `avert`만 아직 응시 붙잡힘 판정을 갖는다 (그것도 M3에서 사라진다).
 *  이름은 데이터(anomalies.json)와 함께 움직여야 해서 판정이 다 빠질 때 한 번에 바꾼다 */
...
export const TEXT = {
  ...
  // ⭐ 직시 힌트 2종은 v0.11.50에 사라졌다 — 짚는 동사가 없으므로 가르칠 것이 없다.
  // 남은 힌트는 걷기와 외면 둘뿐이고, 외면도 그 판정과 함께 사라질 예정이다 (game.md M3)
