...
「약효가 다하기 전에 (Before the Meds Wear Off)」 — 새벽 골목을 걷는 **심리 호러 / 환각 관찰**
브라우저 게임 (Three.js + TypeScript + Vite, 싱글플레이). 약(FF-1204)을 사러 처음 가보는
골목을 직진하며, 있어서는 안 되는 것(환각)을 **직시하면 사라지고**, 지나치면 **골목이 접혀**
(늘어나는 남은 거리 + 증식) 긴장을 만든다. 깊이 = 병세(꺼져가는 가로등 + 통증 비네트).
(2026-08-02 컨셉 전환 v0.9.0 — game.md 결정 기록. 레포명 fries-get-cold는 URL 보존용 유지)
...
- **어포던스·정직한 판정** — 환각은 공정하게 보여야 한다 (배치 3원칙: 광원 안쪽·관찰
...
- **콘텐츠는 데이터, 로직은 시스템** — 환각 추가에 코드 수정이 필요하면 설계 실패
