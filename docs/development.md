...
- **조준 표시가 이 도구의 핵심이다.** 화면 중앙이 맞힌 메시를
  ...
  이름 경로·크기·색·거리·좌표. 좌표는 미터와 함께 **`-L*0.296` 배수 표기**를 붙인다:
  월드 코드가 위치를 전부 그렇게 쓰기 때문에(`box(..., -L * 0.32, t4)`) 화면에서 본 것을
  **그대로 grep할 수 있다.** 이름은 `world/index.ts`가 붙인다 — 새 그룹에는 그때그때 붙일 것
- three의 레이캐스터는 **안 보이는 객체도 맞힌다.** 이 게임은 테마 5개가 같은 좌표를
  공유하므로 조상까지 `visible`을 확인한다 (occlusionReport와 같은 함정 — '배운 것' 참조)
- 모드를 끄면 **날아간 자리에 내린다** (벽 밖·다음 구간으로는 못 내리게 클램프)
- 검증 훅: `__fries.admin.on() / cam() / toggle()` — 비행 중에는 player가 아니라 카메라가 움직인다
