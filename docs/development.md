...
└── src/
    ...
    ├── balance.ts            # 깊이·늘어남 판정 순수 로직 + 밤 시뮬레이션 (게임·테스트 공유)
...
- **입력 분기** — PC는 포인터락 + WASD, 모바일은 드래그(시점) + 걷기 버튼 홀드
  (pointerdown/up + setPointerCapture). **탭 판정과 크로스헤어는 v0.11.50에 사라졌다** —
  클릭에 남은 일은 포인터락 재획득뿐이다
...
빨간불에는 정지선에서 기다린다. **흔적에는 아무것도 하지 않는다** — v0.11.50부터
짚는 동사가 없어서 하네스의 지적 로직(`tap`·`gazeAim`)도 함께 사라졌다.
...
index.html에도 hud.ts에도 흔적이 없다 — 지우려면 이 파일과 main.ts의 네 자리
(import · `new Admin` 블록 · `updateWalk` 앞머리 분기 · `tick` 한 줄)면 된다.
(다섯째였던 `tryPoint` 가드는 그 함수와 함께 v0.11.50에 사라졌다.)
...
- **관리자 모드에서는 판정이 전부 멈춘다** — 붙잡힘·치임·늘어남·구간 전환·깊이.
  보러 들어간 것만으로 상태가 오염되면 도구가 아니다
  (예전에는 클릭의 '빈 지적'까지 막아야 했다 — 그 비용은 v0.11.50에 사라졌다)
...
`renderer.shadowMap.enabled = true` · `PCFSoftShadowMap`. **캐스터는 방향광(해) 하나뿐이다.**

- 포인트라이트 10개는 전부 제외한다 — 큐브 섀도라 **하나당 6패스**(전부 켜면 61패스)이고,
  밤은 그 등들이 만드는 시간인데 하필 가장 비싸고 가장 안 보인다
- `shadowMap.enabled`는 **켜 둔 채** 광원의 `castShadow`로 여닫는다 (`setMorning`) —
  런타임 토글은 전 재질 셰이더를 재컴파일해 그 프레임이 튄다
- 캐스터/리시버 지정은 `createWorld`에서 한 번, `MeshStandardMaterial`인 메시만
  (하늘 돔·검은 안개 판은 조명을 안 받고, 반지름 80m 돔을 캐스터로 넣으면 세계를 덮는다)
- 그림자 카메라는 원점이 아니라 **구간 중앙(`SUN_FOCUS`)** 에 ±28로 선다 → 2.7cm/texel.
  ⚠ `moon.position`은 그 지점에서의 **방향 오프셋**으로 다룬다(`aimSun`): 방향은
  `position − target`이라 target만 옮기면 시간대별 각도가 전부 무너진다
- 비용 실측(SwiftShader · 상태만 옮겨 90프레임): 퇴근길 16.6~16.7ms · 밤 16.6ms →
  **차 0.05ms/frame**, 셋 다 vsync 상한. ⚠ 절대값은 실기기와 무관하고 **모바일은 못 쟀다**
...
- ⚠ `surfaceMat()`은 넘겨받은 텍스처를 **clone한다** (면마다 반복 수가 달라야 하므로).
  루프 안에서 `wallTexture()`를 부르면 호출마다 원본+복제 두 장이 생긴다 — 기반 텍스처는
  루프 밖에서 한 번 만들고, 치수가 같으면 재질을 재사용한다 (`surfRole`의 캐시)
